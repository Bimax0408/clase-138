...
function game() {
  ...
  if (gameConfig.status === 'start') {
    ...
    textSize(40);
    textAlign(CENTER);
    text("Da clic en el boton Empezar para jugar! 🎮 ", gameConfig.screenX / 2, gameConfig.screenY / 2);
    textSize(40);
  ...
  if (gameConfig.status === 'gameover') {
    ...
    textSize(15);
    text("Presiona Espacio para volver a intentar", gameConfig.screenX / 2, gameConfig.screenY / 2 + 135);
    textSize(40);
...
function changeGameStatud(character) {
  if (empezado && gameConfig.status === "start") {
...
function manualControl(character) {
  ...
  if (character.live) {
    if (keyDown(control.left) || narizx > 207) {
      character.velocity.x -= gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(-1);
    }

    else if (keyDown(control.right) || narizx < 138) {
      character.velocity.x += gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(1);
    }
...
function jumping(character) {
  if ((keyWentDown(control.up) && character.live) || (touchIsDown && character.live) || (narizy < 170 && character.live)) {
...
function scores(character) {
  ...
  text("Puntajes: " + round(gameConfig.scores), 20, 40);
  ...
  if (mario.live == false && mario.liveNumber != 0) {
    ...
    if (character.liveNumber == 1) {
      text("Vida", gameConfig.screenX / 2, gameConfig.screenY / 2);
